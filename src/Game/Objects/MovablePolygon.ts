...
{
  centerOrigin() {
    const sum: Vector2 = new Vector2()
    this._points.forEach((x) => sum.add(x))
    const mid = new Vector2(sum.x / this._points.length, sum.y / this._points.length)
    this._points.forEach((element) => {
      element.sub(mid)
    })
    this.transformation.translate(mid.x, mid.y)
    // console.log('points: ' + JSON.stringify(this._points))
  }
  resetTransoform() {
    this.transformation = new Matrix3(1, 0, 0, 0, 1, 0, 0, 0, 1)
  }
