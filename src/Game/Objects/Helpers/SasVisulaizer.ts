...
class SasVisulaizer extends GameObject implements IDrawable {
  ...
  draw(ctx: CanvasRenderingContext2D): void {
    ...
    this._obj2.getNormalsForCollision()//.splice(1,1)
...
const additon=200;
  ...
  const minMax2: minMax = this._obj2.castObjectOntoNormal(N)
    ...
    const collsionVector: Vector2 | null = AlgorithmsHelpers.SasCollision(this._obj1 as unknown as ICollidable, this._obj2 as unknown as ICollidable)
    if (collsionVector !== null) {
     ...
     DrawHelpers.drawLine(
      ...
      if(typeof (this._obj2 as unknown as Polygon).getTransformedPoints==="function")
        {
          DrawHelpers.drawLines(ctx,(this._obj2 as unknown as Polygon).getTransformedPoints(),4,"yellow")
        ...
        if(typeof (this._obj2 as unknown as Sphere).radius==="number")
          {
            DrawHelpers.drawCircle(ctx,(this._obj2 as unknown as Sphere).center.x,(this._obj2 as unknown as Sphere).center.y,(this._obj2 as unknown as Sphere).radius,"yellow")
          }
