...
export default class CollidableSpringLine extends SpringLine implements ICollidable{
    ...
    castObjectOntoNormal(N: Vector2): minMax {
        ...
        const Normal:Vector2=CalculationHelpes.getNormalOfVector(this._node1.position.clone().sub(this._node2.position));
        const move=Normal.clone().normalize().multiplyScalar(this.width/2);
